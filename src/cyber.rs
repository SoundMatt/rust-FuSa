...
fn is_in_source_dirs(rel: &str, dirs: &[String]) -> bool {
    if dirs.is_empty() || dirs == ["."] { return true; }
    dirs.iter().any(|d| {
...
impl Rule for RuleHardcodedCredentials {
    fn id(&self) -> &str { "CYBER001" }
    fn description(&self) -> &str { "CWE-798: Hardcoded credentials expose secrets in source code." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let patterns: &[(&str, &str)] = &[
            ...
            ("private_key", "private key"), ("access_token", "token"),
            ("auth_token", "token"),
        ];
        ...
        for file in rust_sources(root, cfg) {
            ...
            for (i, line) in content.lines().enumerate() {
                ...
                if lower.trim_start().starts_with("//") { continue; }
...
impl Rule for RuleSqlInjection {
    fn id(&self) -> &str { "CYBER002" }
    fn description(&self) -> &str { "CWE-89: SQL query built from string concatenation risks injection." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let sql_kw: &[&str] = &["SELECT ", "INSERT ", "UPDATE ", "DELETE ", "DROP ", "CREATE "];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            let content = std::fs::read_to_string(&file)
            ...
            for (i, line) in content.lines().enumerate() {
                ...
                let has_interp = trimmed.contains("format!(") || trimmed.contains("+ &") || trimmed.contains("+ \"");
...
impl Rule for RulePathTraversal {
    fn id(&self) -> &str { "CYBER003" }
    fn description(&self) -> &str { "CWE-22: Paths constructed from external input without canonicalization risk traversal." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                if (trimmed.contains("PathBuf::from(") || trimmed.contains("Path::new("))
                    && (trimmed.contains("args[") || trimmed.contains("input") || trimmed.contains("request"))
                {
...
impl Rule for RuleWeakRandom {
    fn id(&self) -> &str { "CYBER004" }
    fn description(&self) -> &str { "CWE-330: Non-cryptographic RNG is predictable and unsuitable for security tokens." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let weak: &[&str] = &["rand::random()", "thread_rng()", "SmallRng", "StdRng::seed_from_u64"];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            let content = std::fs::read_to_string(&file)
...
impl Rule for RuleUncheckedArithmetic {
    fn id(&self) -> &str { "CYBER005" }
    fn description(&self) -> &str { "CWE-190: Integer overflow in debug builds panics; in release builds wraps silently." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                if (trimmed.contains("len()") || trimmed.contains("count()"))
                    ...
                    && !trimmed.contains("checked_") && !trimmed.contains("saturating_")
...
impl Rule for RuleCleartextHttp {
    fn id(&self) -> &str { "CYBER006" }
    fn description(&self) -> &str { "CWE-319: Cleartext HTTP transmits data without encryption." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
...
impl Rule for RuleCommandInjection {
    fn id(&self) -> &str { "CYBER007" }
    fn description(&self) -> &str { "CWE-78: Constructing shell commands from variables risks OS command injection." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
...
impl Rule for RuleDeprecatedCrypto {
    fn id(&self) -> &str { "CYBER008" }
    fn description(&self) -> &str { "CWE-327: MD5 and SHA-1 are cryptographically broken." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let weak: &[(&str, &str)] = &[
            ("md5", "MD5"), ("sha1", "SHA-1"), ("des::", "DES"), ("rc4::", "RC4"),
        ];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            ...
            for (i, line) in content.lines().enumerate() {
                ...
                for (pat, name) in weak {
                    if lower.contains(pat) && (lower.contains("use ") || lower.contains("extern crate")) {
                        findings.push(Finding::new(
                            self.id(), Severity::Error,
...
impl Rule for RuleSensitiveLogging {
    fn id(&self) -> &str { "CYBER009" }
    fn description(&self) -> &str { "CWE-532: Logging sensitive data (passwords, tokens, keys) leaks credentials." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let sensitive: &[&str] = &["password", "passwd", "secret", "api_key", "token", "private_key", "credential"];
        let log_macros: &[&str] = &["println!", "eprintln!", "log::info!", "log::debug!", "log::warn!", "log::error!", "tracing::info!", "tracing::debug!"];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            let content = std::fs::read_to_string(&file)
...
impl Rule for RuleUnvalidatedDeserialize {
    fn id(&self) -> &str { "CYBER010" }
    fn description(&self) -> &str { "CWE-502: Deserialising untrusted data without schema validation risks unexpected behaviour." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let patterns: &[&str] = &[
            "serde_json::from_reader(", "serde_json::from_slice(", "serde_json::from_str(",
            "bincode::deserialize(", "toml::from_str(",
        ];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
...
impl Rule for RuleUncheckedSliceIndex {
    fn id(&self) -> &str { "CYBER011" }
    fn description(&self) -> &str { "CWE-125: Direct slice indexing panics on out-of-bounds in debug; UB in unsafe." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                if let Some(bracket_pos) = trimmed.find('[') {
                    ...
                    if let Some(close) = trimmed[bracket_pos..].find(']') {
                        ...
                        if !is_literal && !is_range && !before.trim_end().ends_with("if")
                            && !before.trim_end().ends_with("while")
                            && !trimmed.contains(".get(")
                            && !trimmed.contains("//fusa:safe")
                            && before.chars().last().map(|c| c.is_alphanumeric() || c == '_').unwrap_or(false)
                        {
                            let prev = if i > 0 { lines[i - 1].trim() } else { "" };
                            if !prev.contains("//fusa:safe") {
                                findings.push(Finding::new(
...
impl Rule for RuleUnboundedAlloc {
    fn id(&self) -> &str { "CYBER012" }
    fn description(&self) -> &str { "CWE-400: Allocating with user-controlled size without a cap risks resource exhaustion." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let patterns: &[&str] = &[
            "Vec::with_capacity(", "String::with_capacity(",
            "vec![0; ", "vec![Default::default(); ",
        ];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
                    ...
                    if trimmed.contains(pat) {
                        ...
                        if let Some(start) = trimmed.find(pat) {
                            ...
                            if let Some(end) = trimmed[arg_start..].find(')') {
                                ...
                                if !is_literal && !trimmed.contains("MAX") && !trimmed.contains("max") && !trimmed.contains("//fusa:safe") {
                                    let prev = if i > 0 { lines[i - 1].trim() } else { "" };
                                    if !prev.contains("//fusa:safe") {
                                        findings.push(Finding::new(
                                            self.id(), Severity::Warning,
...
impl Rule for RuleTlsBypass {
    fn id(&self) -> &str { "CYBER013" }
    fn description(&self) -> &str { "CWE-295: Disabling TLS certificate verification enables MITM attacks." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let patterns: &[&str] = &[
            "danger_accept_invalid_certs", "accept_invalid_certs", "verify_peer(false)",
            "danger_accept_invalid_hostnames", "disable_built_in_roots",
        ];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
...
impl Rule for RuleToctouCheck {
    fn id(&self) -> &str { "CYBER014" }
    fn description(&self) -> &str { "CWE-367: Checking a resource and then using it introduces a TOCTOU race." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
...
impl Rule for RuleInsecureFilePerms {
    fn id(&self) -> &str { "CYBER015" }
    fn description(&self) -> &str { "CWE-732: World-writable file permissions allow unauthorised modification." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
...
impl Rule for RuleEnvSecretExposure {
    fn id(&self) -> &str { "CYBER016" }
    fn description(&self) -> &str { "CWE-526: Environment variables containing credentials may be logged or inspected." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let sensitive: &[&str] = &["PASSWORD", "SECRET", "TOKEN", "API_KEY", "PRIVATE_KEY", "AUTH"];
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            let content = std::fs::read_to_string(&file)
...
impl Rule for RulePathFromUserInput {
    fn id(&self) -> &str { "CYBER017" }
    fn description(&self) -> &str { "CWE-22: Path built by joining with a variable may enable directory traversal." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
...
impl Rule for RuleManuallyDrop {
    fn id(&self) -> &str { "CYBER018" }
    fn description(&self) -> &str { "CWE-415: ManuallyDrop bypasses Rust's automatic memory management, risking double-free." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
...
impl Rule for RuleFormatWithExternal {
    fn id(&self) -> &str { "CYBER019" }
    fn description(&self) -> &str { "CWE-134: Using external input as a format string template risks injection." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                for mac in &["format!(", "println!(", "eprintln!(", "writeln!(", "write!("] {
                    if trimmed.contains(mac) {
                        if let Some(pos) = trimmed.find(mac) {
                            ...
                            if !after.starts_with('"') && !after.starts_with('\'')
                                && !after.starts_with("std::") && after.len() > 1
                            {
                                let prev = if i > 0 { lines[i - 1].trim() } else { "" };
                                if !prev.contains("//fusa:safe") {
                                    findings.push(Finding::new(
...
impl Rule for RuleUncheckedFromUtf8 {
    fn id(&self) -> &str { "CYBER020" }
    fn description(&self) -> &str { "CWE-20: from_utf8_unchecked skips UTF-8 validation and is UB on invalid input." }
    fn run(&self, root: &Path, cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let mut findings = Vec::new();
        for file in rust_sources(root, cfg) {
            let rel = rel_path(root, &file);
