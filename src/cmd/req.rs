...
use crate::config::{load_reqs, Requirement, ReqsFile};
...
pub fn run(args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    ...
    match subcmd {
        ...
        other => {
            ...
            writeln!(stderr, "Usage: rsfusa req [show|import|export] [--dir <path>]").ok();
            EXIT_USAGE
        }
    }
}

fn cmd_show(root: &PathBuf, args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    ...
    let cfg = load(&root.join(".fusa.json")).ok().unwrap_or_else(|| {
        crate::config::FusaConfig::new("project", "generic")
    });
    ...
    writeln!(stdout, "{:<16} {:<40} {:<12} {}", "ID", "Title", "Traced", "Tested").ok();
    ...
    for req in &reqs.requirements {
        let (traced, tested) = if let Some((ref matrix, _)) = trace_data {
            ...
            let tested = matrix.tags.iter().any(|t| t.requirement_id == req.id
...
fn cmd_import(args: &[String], root: &PathBuf, stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    let file = match parse_flag(args, "--file").or_else(|| {
    ...
    let mut existing = if reqs_path.exists() {
        load_reqs(&reqs_path).unwrap_or(ReqsFile { requirements: vec![] })
    } else {
        ReqsFile { requirements: vec![] }
    };
    ...
    match std::fs::write(&reqs_path, json + "\n") {
        Ok(_) => {
            writeln!(stdout, "Imported {added} requirements to {}", reqs_path.display()).ok();
            EXIT_OK
        }
        Err(e) => {
...
fn cmd_export(root: &PathBuf, args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    let reqs_path = root.join(".fusa-reqs.json");
    let reqs = match load_reqs(&reqs_path) {
        Ok(r) => r,
        Err(e) => {
    ...
    if format == "csv" {
        ...
        let out_path = parse_flag(args, "--output").unwrap_or_else(|| "requirements.csv".to_string());
