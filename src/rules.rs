...
impl Rule for RuleConfigPresent {
    ...
    fn description(&self) -> &str {
        "Project must have a .fusa.json configuration file."
    }
...
impl Rule for RuleCargoTomlPresent {
    ...
    fn description(&self) -> &str {
        "Project must be a Rust crate or workspace (Cargo.toml present)."
    }
    fn run(&self, root: &Path, _cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
...
impl Rule for RuleLicensePresent {
    ...
    fn description(&self) -> &str {
        "Project must have a LICENSE file for IP clarity in safety cases."
    }
    fn run(&self, root: &Path, _cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
...
impl Rule for RuleReadmePresent {
    ...
    fn description(&self) -> &str {
        "Project must have a README for assessors and integrators."
    }
    fn run(&self, root: &Path, _cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
...
impl Rule for RuleCiPresent {
    ...
    fn description(&self) -> &str {
        "Project must have a CI configuration for automated verification."
    }
    fn run(&self, root: &Path, _cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        let candidates = [
...
impl Rule for RuleReqsPresent {
    ...
    fn description(&self) -> &str {
        "Project should have a .fusa-reqs.json requirements registry."
    }
    fn run(&self, root: &Path, _cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
...
impl Rule for RuleNoDuplicateReqs {
    ...
    fn description(&self) -> &str {
        "Requirement IDs in .fusa-reqs.json must be unique (§1.2.2)."
    }
    fn run(&self, root: &Path, _cfg: &FusaConfig) -> Result<Vec<Finding>, String> {
        ...
        let reqs = crate::config::load_reqs(&path).map_err(|e| format!("FUSA007: {e}"))?;
