...
pub fn run(args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    if args.is_empty() {
        writeln!(stderr, "rsfusa sign: usage: rsfusa sign [--verify] [--keygen] --key <keyfile> <file>").ok();
        return EXIT_USAGE;
    }
...
fn cmd_keygen(_args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    ...
    match output {
        Ok(o) if o.status.success() => {
            ...
            writeln!(stdout, "Keep this file secret and never commit it to source control.").ok();
            EXIT_OK
        }
        _ => {
            // Fallback: write 32 zero bytes (not secure, warn user)
            writeln!(stderr, "rsfusa sign keygen: /dev/urandom not available; using placeholder key").ok();
...
fn cmd_sign(args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    ...
    let target = match args.iter().find(|a| !a.starts_with("--") && a.as_str() != key_path) {
        Some(t) => t.clone(),
        None => {
...
fn cmd_verify(args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    ...
    let target = match args.iter().find(|a| !a.starts_with("--") && a.as_str() != key_path) {
        Some(t) => t.clone(),
        None => {
            writeln!(stderr, "rsfusa sign --verify: target file argument is required").ok();
            return EXIT_USAGE;
        }
    };
    ...
    let sig_hex = match std::fs::read_to_string(&sig_path) {
        ...
        Err(e) => {
            writeln!(stderr, "rsfusa sign --verify: read signature {sig_path}: {e}").ok();
            return EXIT_RUNTIME;
        }
    };
    ...
    let expected_bytes = match hex::decode(&sig_hex) {
        Ok(b) => b,
        Err(_) => {
            writeln!(stderr, "rsfusa sign --verify: invalid signature format in {sig_path}").ok();
            return EXIT_RUNTIME;
        }
    };
